...
gnome_druid_page_edge_init (GnomeDruidPageEdge *druid_page_edge)
{
	...
	druid_page_edge->canvas = gnome_canvas_new ();
	gtk_widget_set_usize (druid_page_edge->canvas, DRUID_PAGE_WIDTH, DRUID_PAGE_HEIGHT);
	gtk_widget_show (druid_page_edge->canvas);
	gnome_canvas_set_scroll_region (GNOME_CANVAS (druid_page_edge->canvas), 0.0, 0.0, DRUID_PAGE_WIDTH, DRUID_PAGE_HEIGHT);
	gtk_container_add (GTK_CONTAINER (druid_page_edge), druid_page_edge->canvas);
...
gnome_druid_page_edge_configure_size (GnomeDruidPageEdge *druid_page_edge, gint width, gint height)
{
	...
	gnome_canvas_item_set (druid_page_edge->textbox_item,
			       "x1", watermark_width,
			       "y1", LOGO_WIDTH + GNOME_PAD * 2.0,
			       "x2", (gfloat) width,
			       "y2", (gfloat) height,
			       "width_units", 1.0, NULL);
	gnome_canvas_item_set (druid_page_edge->logoframe_item,
			       "x1", (gfloat) width - LOGO_WIDTH -GNOME_PAD,
			       "y1", (gfloat) GNOME_PAD,
			       "x2", (gfloat) width - GNOME_PAD,
			       "y2", (gfloat) GNOME_PAD + LOGO_WIDTH,
			       "width_units", 1.0, NULL);
	gnome_canvas_item_set (druid_page_edge->logo_item,
			       "x", (gfloat) width - GNOME_PAD - LOGO_WIDTH,
			       "y", (gfloat) GNOME_PAD,
			       "width", (gfloat) LOGO_WIDTH,
			       "height", (gfloat) LOGO_WIDTH, NULL);
	gnome_canvas_item_set (druid_page_edge->watermark_item,
			       "x", 0.0,
			       "y", watermark_ypos,
			       "width", watermark_width,
			       "height", watermark_height,
			       NULL);
	gnome_canvas_item_set (druid_page_edge->title_item,
			       "x", 15.0,
			       "y", (gfloat) GNOME_PAD + LOGO_WIDTH / 2.0,
			       "anchor", GTK_ANCHOR_WEST,
			       NULL);
	gnome_canvas_item_set (druid_page_edge->text_item,
...
gnome_druid_page_edge_construct (GnomeDruidPageEdge *druid_page_edge)
{
	...
	druid_page_edge->background_item =
		gnome_canvas_item_new (gnome_canvas_root (GNOME_CANVAS (druid_page_edge->canvas)),
	...
	druid_page_edge->logoframe_item =
		gnome_canvas_item_new (gnome_canvas_root (GNOME_CANVAS (druid_page_edge->canvas)),
				       gnome_canvas_rect_get_type (),
				       "fill_color_rgba", fill_color,
				       NULL);

	druid_page_edge->logo_item =
		gnome_canvas_item_new (gnome_canvas_root (GNOME_CANVAS (druid_page_edge->canvas)),
	...
	druid_page_edge->watermark_item =
		gnome_canvas_item_new (gnome_canvas_root (GNOME_CANVAS (druid_page_edge->canvas)),
				       gnome_canvas_pixbuf_get_type (),
				       "x_set", TRUE, "y_set", TRUE,
				       NULL);
				       ...
				       GtkAllocation           *allocation)
{
	...
	gnome_canvas_set_scroll_region (GNOME_CANVAS (GNOME_DRUID_PAGE_EDGE (widget)->canvas),
					0.0, 0.0,
					allocation->width,
					allocation->height);
